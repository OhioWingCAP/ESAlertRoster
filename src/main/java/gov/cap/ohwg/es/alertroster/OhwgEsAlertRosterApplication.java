...
public class OhwgEsAlertRosterApplication {
    ...
    public void onApplicationEvent() {
        unitRepo.deleteAll();
        FlatFileItemReader<Unit> unitReader = new FlatFileItemReader<>();
        unitReader.setLinesToSkip(1);
        unitReader.setResource(new ClassPathResource("/data/Organization.txt"));
        DefaultLineMapper<Unit> lineMapper = new DefaultLineMapper<>();
        DelimitedLineTokenizer tokenizer = new DelimitedLineTokenizer();
        tokenizer.setNames(new String[]{"orgid", "region", "wing", "unit", "nextLevel", "name", "type",
                "dateChartered", "status", "scope", "usrID", "dateMod", "firstUsr", "dateCreated", "dateReceived",
                "orgNotes"});
        lineMapper.setLineTokenizer(tokenizer);
        BeanWrapperFieldSetMapper<Unit> mapper = new BeanWrapperFieldSetMapper<>();
        mapper.setTargetType(Unit.class);
        lineMapper.setFieldSetMapper(mapper);
        unitReader.setLineMapper(lineMapper);
        unitReader.open(new ExecutionContext());
        Unit unit = null;
        try {
            while ((unit = unitReader.read()) != null) {
                unitRepo.save(unit);
            }
        } catch (Exception e) {
            e.printStackTrace();
